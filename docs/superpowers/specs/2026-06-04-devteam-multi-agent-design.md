...
DevTeam 是一个基于 LangGraph 的多Agent协同系统，模拟真实软件开发团队的工作流程。用户输入自然语言需求（如"做一个待办事项Web应用"），系统自动完成从需求分析到代码交付的全流程。
...
详见 [2026-06-04-devteam-implementation-plan.md](./2026-06-04-devteam-implementation-plan.md)
...
**当前方案（开发阶段）：** 自动生成+持久化到`.devteam_secret`文件

**问题：** 以下场景会导致secret变化，旧token全部失效：
- 删除`.devteam_secret`文件
...
```python
# 环境变量注入，不写入代码或配置文件
DEVTEAM_LLM_API_KEY = os.environ.get("DEVTEAM_LLM_API_KEY")
```
...
| 数据 | 当前存储 | 部署建议 |
|------|---------|---------|
| 用户数据 | SQLite (`devteam/data/devteam_memory.db`) | PostgreSQL |
| 项目文件 | 本地目录 (`devteam/projects/`) | 对象存储(S3/MinIO) |
| 设置 | JSON文件 (`devteam/data/settings.json`) | 数据库 |
| JWT Secret | 文件 (`devteam/.devteam_secret`) | 环境变量/密钥管理 |
