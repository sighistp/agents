...
from blueprint.agents.tools import DEVELOPER_TOOLS, serialize_call, get_call_name, get_call_args
from blueprint.agents import tool_executor
import asyncio
...
- 每个函数加简短注释说明用途（1-2 行）
- 关键逻辑加行内注释
- 变量命名清晰（不用 a, b, temp 等模糊名称）
- 代码结构清晰：条件判断、循环、异常处理都要有适当缩进
...
async def developer_agent(state: dict) -> dict[str, Any]:
    ...
    try:
        for step in range(MAX_STEPS):
            ...
            all_tool_calls.extend([get_call_name(tc) for tc in response.tool_calls])
