...
from blueprint.utils.guard import check_injection
...
## 常见错误（避免）
❌ 需求明确时（如"做一个计算器"）还设 needs_clarification: true
❌ 用户故事没有验收标准
❌ 需求太模糊时不澄清直接拆解

## 重要规则
- user_stories 不能为空（至少一个故事）
- 每个用户故事必须有至少一个验收标准
...
async def pm_agent(state: ProjectState) -> dict:
    ...
    """
    from blueprint.agents.discussion import get_discussion_config, proposer_critic_discuss
