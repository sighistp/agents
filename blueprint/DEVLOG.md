...
## Phase 83：文件消失 + 旧项目消息泄漏（2026-06-23）

**来源：** 真实运行测试（紫色计算器）

### Bug 1：文件消失（`{}` 在 JS 是 truthy）

**现象：** 运行结束后文件面板为空，刷新后文件出现
**根因：** `project_done` 消息发送 `files: output.get("files", {})`。当 deliver 节点 output 没有 `files` 字段时返回 `{}`。JavaScript 中 `Boolean({})` 是 `true`，前端执行 `projectStore.files = {}` 清空了文件列表。
**修复：** `agent_update` 和 `project_done` 两处都加 `Object.keys(data.data.files).length > 0` 检查
**文件：** `frontend/src/composables/useWebSocket.js`

### Bug 2：新建对话第一句是"项目完成！"

**现象：** 每次点"新建对话"，聊天区第一行显示"项目完成！"
**根因：** WebSocket 消息过滤逻辑 `data.project_id && activeProjectId && data.project_id !== activeProjectId` — 当用户点"新建对话"后 `activeProjectId = null`（falsy），条件短路为 false，旧项目的 `project_done` 消息泄漏进来。
**修复：** 简化为 `data.project_id && data.project_id !== activeProjectId`，当 `activeProjectId` 为 null 时正确拒绝所有带 project_id 的旧消息
**文件：** `frontend/src/composables/useWebSocket.js`
...
## Phase 83b：全量视图重复修复（2026-06-23）

**现象：** 全量模式下每个 Agent 的对话包含所有之前 Agent 的消息
**根因：** `save_conversation` 存了 `dict_msgs` 中所有 `role == "assistant"` 的消息，不只是当前 Agent 的
**修复：** 过滤为只存 `m.get("name") == node_name`（当前 Agent）+ `role == "tool"` 的消息
**文件：** `api/websocket.py`

---

## 本次会话总修复清单（Phase 80-83）

| # | 问题 | 修复 |
|---|------|------|
| 1 | auth_enabled=False | 改为 True |
| 2 | CORS 通配 | 从 config 读取 |
| 3 | API 端点无认证 | 全部加 verify_api_key |
| 4 | Webhook SSRF | _validate_webhook_url() |
| 5 | JWT Secret 无权限 | os.chmod(0o600) |
| 6 | base_url 无校验 | _validate_base_url() |
| 7 | SQLite 连接泄漏 | close_all() |
| 8 | 消息无滑动窗口 | trim_messages() |
| 9 | requirements 无版本 | 全部加版本范围 |
| 10 | test_passed 默认 True | 改为 False |
| 11 | Severity 枚举不一致 | 统一 critical/important/minor |
| 12 | meta.json 无 encoding | 加 encoding="utf-8" |
| 13 | Content-Disposition 注入 | _sanitize_filename() |
| 14 | except:pass 清理 | 审计 + 白名单 |
| 15 | seenMessageIds 无限增长 | LRU 1000 |
| 16 | CSS 硬编码 width | max-width + width: 100% |
| 17 | 安检查拦截正常 import | 从危险模式移除 |
| 18 | KeyError human_confirm | 添加路由映射 |
| 19 | had_execution_errors sticky | 每次成功后重置 |
| 20 | Developer 文件丢失 | merge 旧文件+新文件 |
| 21 | 中文 test_passed 失败 | 加中文正则 |
| 22 | GBK 编码崩溃 | 加 encoding="utf-8" |
| 23 | 文件消失 | {} 是 truthy，加 length 检查 |
| 24 | 旧项目消息泄漏 | 简化 project_id 检查 |
| 25 | 精简/全量切换 | localStorage 持久化 |
| 26 | 全量视图重复 | 只存当前 Agent 消息 |

**测试：411 后端 + 103 前端 = 514 全过**

---

## Phase 84：interrupt 确认框 + 全量视图卡死 + 项目完成消息（2026-06-23）

**来源：** 真实运行测试（紫色计算器，第二轮）

### Bug 1：interrupt 确认框不弹

**现象：** 后端日志显示 `Human confirm: calling interrupt()` 但前端没弹确认框
**根因：** astream 循环结束后，代码检查 `final_status == "running"` 决定发 `interrupt` 还是 `project_done`。但 `human_confirm_node` 没设 `status = "running"`，`final_status` 默认 `"delivered"`，走了 `project_done`。
**修复：** astream 结束后调用 `app.get_state(config)` 检查 `graph_state.next` 是否有 pending 节点。如果有，从 `graph_state.tasks` 提取 interrupt value 并发送 `interrupt` 消息。
**文件：** `api/websocket.py`

### Bug 2：全量视图卡死（疯狂请求 API）

**现象：** 刷新后页面卡死，终端显示大量 `GET /api/projects/.../conversations` 请求
**根因：** localStorage 里 `viewMode='full'`，但页面加载后 `fullMessages` 为空且没有自动加载。`switchView` 没有 loading 锁，可能被反复调用。
**修复：**
- 加 `fullLoading` ref 防止重复请求
- 加 `onMounted` 自动加载全量视图
- 加 `finally` 块重置 loading 状态
**文件：** `frontend/src/components/ChatPanel.vue`

### Bug 3：新建对话后显示"项目完成！"

**现象：** 点"新建对话"后聊天区第一行显示"项目完成！"
**根因：** `project_done` handler 调用 `projectStore.addMessage()` 添加了 "项目完成！" 消息。刷新后 `state_sync` 从数据库加载旧消息，"项目完成！" 被重复显示。
**修复：** 从 `project_done` handler 中移除 `addMessage` 调用，只更新 `isRunning` 状态和文件列表。
**文件：** `frontend/src/composables/useWebSocket.js`

### Bug 4：精简/全量按钮消失

**现象：** 新建对话后精简/全量切换按钮消失
**根因：** 按钮有 `v-if="projectStore.currentProject?.id"` 条件，新建对话后 `currentProject` 为 null。
**修复：** 去掉 v-if 条件，按钮始终显示。
**文件：** `frontend/src/components/ChatPanel.vue`
...
## Phase 85：全代码审计 + interrupt 逻辑反了（2026-06-23）

**来源：** 全代码审计（5 个子代理并行审查 ChatPanel、useWebSocket、websocket.py、tester.py、developer.py）

### 审计发现（10 个问题）

| 严重度 | 文件 | 问题 | 状态 |
|--------|------|------|------|
| **Critical** | websocket.py | interrupt 检测逻辑反了（has_interrupt=True 发 project_done） | ✅ 已修 |
| **Critical** | websocket.py | graph_state.next 在正常完成时也非空，导致 project_done 永远不发 | ✅ 已修 |
| **High** | ChatPanel.vue | switchView 全量数据不刷新，旧项目数据残留 | 待修 |
| **High** | ChatPanel.vue | clearChat 没重置 viewMode，全量模式卡住 | ✅ 已修 |
| **High** | developer.py | implicit-done 在只读步骤后触发，可能提前结束 | 待修 |
| **Medium** | useWebSocket.js | state_sync 覆盖本地消息 | 待修 |
| **Medium** | developer.py | test_passed 无条件重置为 True | 待修 |
| **Medium** | tester.py | had_execution_errors 每次重置，早期失败被遗忘 | ✅ 已修 |
| **Low** | ChatPanel.vue | IME 中文输入可能触发 premature send | 待修 |
| **Low** | useWebSocket.js | dedup key 截断 100 字符可能误判 | 待修 |

### 修复详情

#### Bug 1：interrupt 逻辑反了（Critical）

**现象：** interrupt 确认框不弹，项目永远不完成
**根因：** `has_interrupt=True` 时走了 `project_done`，`has_interrupt=False` 时走了 `interrupt`。条件反了。
**修复：** 简化逻辑 — astream 循环中 `__end__` 分支发 `project_done` 并 return；循环结束后（说明被 interrupt）发 `interrupt`。
**文件：** `api/websocket.py`

#### Bug 2：graph_state.next 不可靠（Critical）

**现象：** 修了逻辑反了之后，项目还是不完成
**根因：** `graph_state.next` 在正常完成时也非空（LangGraph 内部状态），导致 `has_interrupt` 永远为 True
**修复：** 移除 graph_state.next 检查，改为：astream 循环中 `__end__` = 完成，循环结束 = interrupt
**文件：** `api/websocket.py`

#### Bug 3：clearChat 没重置 viewMode

**现象：** 清空聊天后 UI 卡在空的全量视图
**修复：** clearChat 同时重置 `viewMode = 'brief'` + localStorage
**文件：** `frontend/src/components/ChatPanel.vue`
...
> 基于 Phase 80-85 的实战经验，LangGraph 暴露了以下根本性问题

### 问题 1：astream 不发 `__end__`

**现象：** reviewer approve 后路由到 deliver，但 deliver 从不执行，astream 循环永远不结束
**影响：** 项目永远不完成，用户永远看不到结果
**根因：** LangGraph 的 astream 在某些条件下不发 `__end__` 事件，可能是 checkpointer 状态问题
** workaround：** 无。这是框架层面的 bug。

### 问题 2：state 合并是补丁式替换

**现象：** Developer 只返回本次文件，旧文件丢失
**影响：** Phase 81b 修复 — 所有返回路径手动 merge
**根因：** LangGraph 的 state 合并不是 deep merge，dict 字段被整体替换

### 问题 3：interrupt/resume 依赖 checkpointer

**现象：** MemorySaver 重启丢状态，SqliteSaver 不支持 async
**影响：** Phase 69 暂缓 — 无法使用持久化 checkpointer
**根因：** LangGraph 的 checkpointer API 是 async-first，同步入口函数无法支持

### 问题 4：路由函数和条件边映射是两套系统

**现象：** 路由函数返回字符串，必须在 add_conditional_edges 里手动映射，漏一个 key 就 KeyError
**影响：** Phase 81 修复 — developer 条件边缺 human_confirm 映射

### 问题 5：调试困难

**现象：** 图执行时看不到中间状态，出错了只知道"某个节点崩了"
**影响：** 每次排查都靠猜 + 加日志 + 重启

### 结论

LangGraph 的抽象层太厚，对于 Blueprint 这种需要精细控制的场景，框架本身成了最大的 bug 来源。

**如果重新选：** asyncio 手写状态机 + 简单 dict 状态，100% 可控，0 框架 bug。
