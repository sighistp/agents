"""Tests for the Blueprint configuration module (TDD RED phase)."""
...
class TestDeepSeekEnvOverride:
    """Values can be overridden via BLUEPRINT_ prefixed env vars."""

    def test_env_api_key(self):
        with patch.dict(os.environ, {"BLUEPRINT_DEEPSEEK_API_KEY": "sk-env-key"}):
            from blueprint.config import Settings
            s = Settings()
            assert s.deepseek_api_key == "sk-env-key"

    def test_env_base_url(self):
        with patch.dict(os.environ, {"BLUEPRINT_DEEPSEEK_BASE_URL": "https://env.api.com"}):
            from blueprint.config import Settings
            s = Settings()
            assert s.deepseek_base_url == "https://env.api.com"

    def test_env_model(self):
        with patch.dict(os.environ, {"BLUEPRINT_DEEPSEEK_MODEL": "env-model"}):
            from blueprint.config import Settings
            s = Settings()
...
class TestBaseSettingsBehavior:
    ...
    def test_env_prefix_is_blueprint(self):
        from blueprint.config import Settings
        s = Settings()
