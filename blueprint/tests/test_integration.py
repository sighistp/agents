"""Integration tests for the full Blueprint workflow."""
import json
import pytest
...
async def test_full_workflow_with_mock_llm():
    ...
    pm_response = json.dumps({
        "user_stories": [{"id": "US-001", "title": "Create calculator", "description": "User can add numbers",
                          "acceptance_criteria": ["Given two numbers, when added, then result is correct"],
                          "priority": "high"}],
        "features": [{"name": "Addition", "description": "Basic addition", "priority": "high",
    ...
    architect_response = json.dumps({
        ...
        "api_definitions": [{"method": "POST", "path": "/add", "description": "Add two numbers",
                              "request_body": {"a": "int", "b": "int"}, "response_body": {"result": "int"}}],
        "data_models": []
    })
    ...
    with patch("blueprint.agents.pm.call_llm_async", side_effect=mock_pm_call_llm):
        with patch("blueprint.agents.architect.call_llm_async", side_effect=mock_architect_call_llm):
            with patch("blueprint.agents.developer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=dev_responses):
                with patch("blueprint.agents.tester.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=tester_responses):
                    with patch("blueprint.agents.reviewer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=reviewer_responses):
...
async def test_workflow_stops_on_error():
    ...
    with patch("blueprint.agents.pm.call_llm_async", side_effect=mock_call_llm_error):
...
async def test_workflow_handles_pm_clarification():
    ...
    clarification_response = json.dumps({
        "user_stories": [{"id": "US-001", "title": "Create todo", "description": "User can create todo",
                          "acceptance_criteria": ["Given a form, when submitted, then todo is created"],
                          "priority": "high"}],
        "features": [],
    ...
    with patch("blueprint.agents.pm.call_llm_async", side_effect=mock_call_llm):
...
async def test_workflow_handles_test_failure():
    ...
    responses = [
        ...
        _make_response("", [_make_tool_call("done", {"summary": "1 passed, 1 failed"})]),
    ]

    with patch("blueprint.agents.tester.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=responses):
        with patch("blueprint.agents.tool_executor.execute_tool") as mock_exec:
...
async def test_workflow_iteration_tracking():
    ...
    with patch("blueprint.agents.developer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=responses):
        with patch("blueprint.agents.tool_executor.execute_tool") as mock_exec:
...
async def test_workflow_proposer_critic_integration():
    ...
    with patch("blueprint.agents.discussion.DISCUSSION_CONFIG", mock_config):
        with patch("blueprint.agents.discussion.call_llm_async", side_effect=mock_call_llm):
            with patch("blueprint.agents.discussion.get_discussion_config", return_value=mock_config):
