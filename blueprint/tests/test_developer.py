...
async def test_developer_max_steps():
    ...
    with patch("blueprint.agents.developer.call_llm_with_tools_async", new_callable=AsyncMock, return_value=always_read):
        with patch("blueprint.agents.tool_executor.execute_tool") as mock_exec:
...
async def test_developer_implicit_done_with_files():
    ...
    with patch("blueprint.agents.developer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=mock_llm):
        with patch("blueprint.agents.tool_executor.execute_tool") as mock_exec:
...
async def test_developer_handles_llm_error():
    ...
    with patch("blueprint.agents.developer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=Exception("API Error")):
        result = await developer_agent(state)
