...
def test_tester_system_prompt_exists():
    from blueprint.agents.tester import TESTER_SYSTEM_PROMPT
...
async def test_tester_handles_llm_error():
    ...
    with patch("blueprint.agents.tester.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=Exception("API Error")):
        result = await tester_agent(state)
