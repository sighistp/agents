...
def _projects_dir() -> Path:
    ...
    Honours the BLUEPRINT_PROJECT_DIR env-var so tests can override it.
    Default: {project_root}/projects (same as create_initial_state uses).
    """
    import os

    return Path(os.environ.get(
        "BLUEPRINT_PROJECT_DIR",
...
def get_project_quality(project_id: str):
...
def get_project_diff(project_id: str, a: int = 0, b: int = 0):
...
def get_project_security(project_id: str):
    """Run security scan on project files."""
    _validate_project_id(project_id)
    _read_meta(project_id)
    pdir = _project_dir(project_id)
...
def get_project_traces(project_id: str, agent: Optional[str] = None, iteration: Optional[int] = None):
    """Get agent execution traces, optionally filtered by agent and iteration."""
    _validate_project_id(project_id)
    _read_meta(project_id)
    trace_db = TraceDB()
    traces = trace_db.get_traces(project_id, agent=agent, iteration=iteration)
