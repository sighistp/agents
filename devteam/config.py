...
DEVTEAM_* environment variables or a .env file.
"""
...
class Settings(BaseSettings):
    """Central configuration for the DevTeam multi-agent system."""
...
# Module-level singleton -- importable as `from devteam.config import settings`
settings = Settings()
...
if not settings.llm_api_key:
    import warnings
    warnings.warn(
        "DEVTEAM_LLM_API_KEY 未设置，LLM调用将失败。"
        "请在环境变量或 .env 文件中设置 DEVTEAM_LLM_API_KEY。",
        stacklevel=2,
    )
