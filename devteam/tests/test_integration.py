...
async def test_full_workflow_with_mock_llm():
    ...
    with patch("devteam.agents.pm.call_llm_async", side_effect=mock_pm_call_llm):
        with patch("devteam.agents.architect.call_llm_async", side_effect=mock_architect_call_llm):
            with patch("devteam.agents.developer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=dev_responses):
                with patch("devteam.agents.tester.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=tester_responses):
                    with patch("devteam.agents.reviewer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=reviewer_responses):
...
async def test_workflow_handles_pm_clarification():
    ...
    with patch("devteam.agents.pm.call_llm_async", side_effect=mock_call_llm):
...
async def test_workflow_handles_review_rejection():
    ...
    with patch("devteam.agents.reviewer.call_llm_with_tools_async", new_callable=AsyncMock, side_effect=responses):
        with patch("devteam.agents.tool_executor.execute_tool") as mock_exec:
...
async def test_workflow_proposer_critic_integration():
    ...
    with patch("devteam.agents.discussion.DISCUSSION_CONFIG", mock_config):
        with patch("devteam.agents.discussion.call_llm_async", side_effect=mock_call_llm):
